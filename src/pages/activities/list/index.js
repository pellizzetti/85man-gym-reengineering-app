...
class ActivitiesList extends Component {
  ...
  state = {
    ...
    columns: [
      ...
      },
      {
        ...
        title: 'Instrutor',
        getCellValue: datum => (
          <Text>{(datum.instructor && datum.instructor.name) || 'Nenhum instrutor'}</Text>
        ),
      },
      {
        name: 'edit',
        ...
        getCellValue: (datum) => {
          ...
          return (
            <Button
              ...
              label="Editar"
              onClick={() => history.push(`/activities/edit/${datum.id}`)}
            />
          );
        },
      },
  ...
  loadData = async (currentPage = 0, search = '') => {
    try {
      ...
      } = await api.get('/activities', { params: { currentPage, search } });
      ...
      toast.error('Erro ao buscar atividades da API. :(');
      console.error(err);
      this.setState({
        results: [],
        totalCount: 0,
    ...
    return (
      <Box flex pad="small">
        ...
        <Grid rows={results} columns={columns}>
          ...
          <TableHeaderRow />
          <Toolbar />
          <SearchPanel messages={{ searchPlaceholder: 'Pesquisar atividades...' }} />
